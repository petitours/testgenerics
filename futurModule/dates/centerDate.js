...
// défini des fonctions liées a la gestion d'une plage de date et de sa date centrale (typiquement le timeline du calendrier)

import * as baseDate from './base.js';

// calcul de la date de début et de fin d'une plage de date 
export function twoDateFromCenter(centerDate,plage) {

    const p = Number(plage) //conversion en nombre de la plage si jamais elle arrive en texte
...
//calcul la date centrale d'une plage de date 
...
export function twoDateMonth(date) {
    ...
    let start = new Date( copiedate.setDate(1)  ); //la plage de date commence au premier jour du mois
    start = new Date( start.setHours(0,0,0,0) ); // ca commence aussi à 0h00

    let end = new Date(start); //et la fin le 1 du mois suivant
...
export function twoDateWeek(date) {
    ...
    let start = new Date( copiedate.setHours(0,0,0,0) ); // le debut de la plage est le lundi correspondant à la date centrale avec h min et s remis à 0
    start.setDate(start.getDate() - dayofweek + 1); // suite du start
      
    let end = new Date(start); //et la fin le lundi suivant
...
export function twoDateDay(date) {
    ...
    let end = new Date(start); //et la fin un jour plus tard
