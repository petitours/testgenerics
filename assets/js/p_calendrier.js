import { Timeline, DataSet } from 'vis-timeline'
  ...
  let container = document.getElementById('timelineici');
  ...
  // enable or disable individual manipulation actions
    ...
    // Create a Timeline
    let timeline = new Timeline(container, new DataSet(), options);
...
/*
    function SetPlus1Span () {
      const visible = timeline.getWindow() ; //recupere la plage de temps affichée 
      const ecart = visible.end.getTime() - visible.start.getTime() ; //calcul de l'etendue de la plage de temps affichée 
      ...
      timeline.setWindow(start ,end);    // change la plage de date du timeline    
      timeline.redraw() ; 
    };

    function SetPlus05Span () {
      const visible = timeline.getWindow() ; //recupere la plage de temps affichée 
      const demiecart = ( visible.end.getTime() - visible.start.getTime() ) / 2 ; //calcul de l'etendue de la plage de temps affichée 
